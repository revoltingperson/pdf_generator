...
class Interface(QMainWindow):
    ...
    def __initialize_tool_bar(self):
        ...
        rotate_right.triggered.connect(lambda command: self.controller.transform_image('rotate_right'))
        rotate_left = self.findChild(QAction, 'Rotate_left')
        rotate_left.triggered.connect(lambda command: self.controller.transform_image('rotate_left'))
        flip_horizontally = self.findChild(QAction, 'Flip_horizontally')
        flip_horizontally.triggered.connect(lambda command: self.controller.transform_image('flip_horizontally'))
        flip_vertically = self.findChild(QAction, 'Flip_vertically')
        flip_vertically.triggered.connect(lambda command: self.controller.transform_image('flip_vertically'))
